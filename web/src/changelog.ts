...
export const RELEASES: Release[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      '**If WhatsApp ever stops delivering without saying so, you will be told — and told what actually went wrong.** OpenMasjidOS found a fault where a masjid’s WhatsApp connection could expire quietly, and messages would keep being accepted, and reported as sent, while none of them arrived. This app now checks once an hour whether any of its own messages fell into a period like that, and tells you when it was, how many, and which of four things went wrong. **Nothing is lost when this happens** — every donation, refund and monthly change from that period is in your records exactly as it should be; it was only the notifications that went missing. We deliberately do not re-send them: a burst of old messages to a phone you have only just reconnected is the quickest way to get that number blocked.',
      '**Fixed: a bad afternoon could bury you in identical messages.** If your card keys expire, or Stripe has an outage, every single person who tries to give fails — and you were told about each one. You now get the first one straight away, and the next one an hour later tells you how many times it happened in between (“this has happened 41 more times”). The donations the app finds for you are reported together too: one message saying “3 donations totalling $140 were found and added” rather than up to twenty-five separate ones.',
      '**If you run a school and use OpenMasjid Students, you can now ask parents to cover the card fee.** Turn it on in Students (Payments → who pays the processing fee) and the tuition page shows it as its own line: the tuition, the card fee, and the total, with a plain sentence saying the fee is what the card networks charge and that paying by cash or check at the office avoids it. **It is off unless your school switches it on.** The child’s balance still goes down by the **tuition** — the card fee is never treated as money towards their fees, so a family can’t end up with a stray credit that quietly comes off their next bill.',
      ...
      'A full security and code-health review went over all of the above. It found no new weakness, and it caught one fault in the new notification grid before you saw it: adding a WhatsApp number worked, but ticking any of its boxes silently failed. That is fixed and checked against a running copy of the app. Nothing you have set up needs redoing, and your donations, appeals and monthly plans are all exactly as they were.',
    ],
  },
  {
    version: '0.43.0',
