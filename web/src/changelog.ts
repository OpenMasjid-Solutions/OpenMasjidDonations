...
export const RELEASES: Release[] = [
  {
    ...
    notes: [
      ...
      '**All your notification settings are now in this app, under Settings → Notifications.** Six things you can be told about — a donation arriving, a refund, a monthly donor stopping their gift, donations breaking altogether, a tuition payment that didn’t reach Students, and a donation that was found and added — and for each one you choose who hears it: your OpenMasjidOS inbox, a specific email address, a WhatsApp number, or an approved WhatsApp group. Any combination, or none. Every one has its own **Test** button.',
      'Your OpenMasjidOS inbox is ticked for all six to begin with, and the email and WhatsApp boxes start empty — **WhatsApp is never switched on for you**, since it sends from your masjid’s own number. Beside “a donation was received” there is a *only if it’s at least…* figure, which is worth setting if you would rather not hear about every £2 gift: it is the one notification that happens every single time somebody gives.',
      'If you were already on the Development channel, one change to know about: you used to get a webhook post for every donation whether you had asked for it or not, and now you get one only if you tick that box — but the box also emails you, which is what people were actually asking for. Your WhatsApp settings carry over. If you had listed several numbers, the first one carries over; an approved group is the better way to reach a few people at once, since it is one message rather than five.',
      'Worth remembering: OpenMasjidOS has its own switches too (Settings → Alerts) and both have to be on. That is why the panel says “your OpenMasjidOS inbox” rather than promising you an email — and why the Test button tells you plainly when OpenMasjidOS is the thing holding it back.',
      '**Light mode looks like it should now.** The page behind your panel stayed dark when you switched to light, so the panel itself was pale cards floating on a near-black background — and the text on that background was hard to read. Light mode now has a light background of its own, and each of the nine wallpapers has a light version that keeps its colour: ocean is still blue, forest still green. Dark mode is untouched.',
      '**A full security and code-health review went over the whole app again**, including everything added in the last week. No new weakness was found. It did find two places where the app told you something untrue, and both are now fixed — see the next two notes. The report, and everything still outstanding, is in the project’s own `docs/audit/` folder.',
      '**Fixed: “monthly donors” over WhatsApp counted people who had already stopped.** A monthly gift is cancelled at your card processor, and nothing here is told when that happens — so a plan somebody ended two years ago still looked like a live one. The longer your masjid had been running, the more wrong the answer got: forty ended plans and ten live ones would have been reported as fifty donors giving four times what you actually receive. It now counts only plans that have actually been charged recently, and tells you separately how many have gone quiet, so a smaller figure explains itself instead of just looking smaller.',
      '**Fixed: asking about an appeal over WhatsApp couldn’t reach one past the twelfth.** The list is cut off at twelve so the message stays readable, but the search was being cut off with it — so if you had thirteen appeals, the thirteenth answered “sorry, I didn’t recognise that one” while you were looking straight at it in the panel. Typing part of a name now finds any of your appeals, however many you have, and the list says how many more there are.',
      'Smaller corrections in the same pass: the setup page could be asked to check OpenMasjidOS over and over by anyone on your network, which is now limited like the other pages that do; a busy moment at OpenMasjidOS could make the WhatsApp section claim for a minute that it wasn’t allowed to send, when in fact nothing had answered; and an ordinary bad request from something else on your network no longer writes an error into your server log, where it could hide a real one.',
      'Housekeeping with no effect on how the app runs: the web server library was updated to the version with four security advisories fixed (none of them reachable in the way this app uses it, verified again, and upgraded anyway), one build-time dependency was bumped, a little dead code and unused styling was deleted, and the documentation was brought back in line with the code — including the parts about WhatsApp, which had been written before the notification settings moved.',
    ],
  },
  {
    version: '0.42.0',
