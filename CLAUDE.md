...
- **Never copy code from umbrelOS / `umbrel-apps` (PolyForm-Noncommercial)** — incompatible with AGPL. Reimplement from behavior.
...
### ✅ In scope (v1.0)
- **Public donation site** (no login): one or more donation pages/appeals, each with title, rich content, images, **preset amounts + a custom amount**, one-time and **monthly recurring** options, an optional **cover-the-fees** toggle, branded with the masjid's name/logo/colors.
...
Since **v0.42.0 each appeal may name its own account** (`campaigns.payment_account`), so a Zakat page can settle somewhere separate from the general fund; `''` means "follow the site default", which is exactly the pre-v0.42.0 behavior. See §13 and `docs/ARCHITECTURE.md` → Per-appeal Stripe accounts.
...
**`manifest.yaml`** — as shipped: `id: donations`, `author: OpenMasjid-Solutions`, `license: AGPL-3.0-only`, `icon: icon.svg`, `screenshots: [screenshots/1.svg]`, a long `description`, and the capability flags `sso`, `notifications`, `https`, `stripe`, `domain`, `email`, `whatsapp`, plus `fabric.consumes: [students/billing]`, `alerts:` (**eight** declared ids — the seven events in §8's Settings tab, plus `test`) and `commands:` (**five**, all read-only: `today`, `month`, `totals`, `appeal`, `monthly`).
...
- **Motion** for gentle entrances, button/press springs, and a satisfying (understated) success state after a donation. **Always honor `prefers-reduced-motion`.**
...
- **WhatsApp is an ADMIN channel and never LOAD-BEARING (v0.43.0).** The masjid links their own number via the OpenWA gateway in OpenMasjidOS; we POST `/api/fabric/whatsapp` and never see the gateway, its credentials or the number. Three rules, and none is ours to relax because the risk lands on the masjid's phone number:
  ...
  - **WhatsApp is OFF by default for every event, and that one is not a matter of taste.** The OS alert channel defaults on; WhatsApp never does. An update must not begin sending messages from a masjid's own phone number on their behalf — the ban risk is theirs, not ours. Since v0.44.0 recipients are LISTS (`NotifySettings.emails`/`.whatsapps`) and this rule is what makes the two lists behave differently: a brand-new **email** address starts subscribed to `NEW_EMAIL_EVENTS` (the alerts that cost money or hide a problem), and a brand-new **WhatsApp** row starts subscribed to **nothing at all**. Do not "tidy" that into one shared default. The pre-v0.44.0 `whatsappOn` boolean is gone with the per-event shape it belonged to; its job — an empty tick list rather than a lost number — is now done by the row's `events` array, and the migration deliberately DROPS a number that was stored with the switch off rather than carrying it over unticked (it would otherwise reappear as a destination the admin does not remember, on their own number).
  ...
  - **`202 {queued:true, id}` is the only success, and it means QUEUED.** Never "sent", never a delivery receipt — WhatsApp gives none. So nothing may block on a send, retry a `202` (it is already queued; retrying duplicates), or tell an admin a message was delivered. **And since platform 0.52.0 a queued message may sit for a very long time on purpose**: when the WhatsApp link is down the platform HOLDS messages rather than dropping them, and an admin releases them after re-linking the phone. Nothing here may assume a `202` resolves quickly. Our single `/status/:id` check ~45s later leaves a still-`queued` message as queued and never escalates it, so it raises no false alarms. **But half its original justification is now dead and that is recorded, not dropped:** it rested on records being evicted after 24h, so asking soon was the only time an answer existed. Since 0.51.1-dev.13 a still-queued message keeps its record for as long as it waits, so a message held for hours and then failing after release is looked at once — at 45s, when `queued` was the only truthful answer — and never again. Nothing is misreported (the panel renders only bad outcomes, so a permanently-`queued` row claims nothing) and nothing depends on it, but such a failure is invisible here. **Widening it is Hasan's call, not a tidy-up**, since "ask once, never in a loop" is the invariant. Kiosk and Display hit the same dead premise as a hard-coded 24h give-up and moved to 7 days.
  - **A MESSAGE REPORTED `sent` MAY NEVER HAVE ARRIVED, and this app does not resend (platform 0.52.0).** A masjid's WhatsApp session expired by itself, the platform did not notice for over a day, and every message in that period was accepted, recorded `sent`, and never delivered. `GET /api/fabric/whatsapp/suspect` now reports those windows (`{ok, windows:[{from,to,cause,count,ids,truncated}]}`, epoch ms, scoped to our app id, on the **read** budget so polling costs no sends). **Retention is 7 days after the outage ENDS** (0.51.1-dev.13); it used to answer only while the outage was open, so re-linking the phone destroyed the evidence exactly when an admin came looking. That is what makes hourly polling sufficient, and it is why `addWhatsAppGap` dedupes: a window is re-reported on ~168 consecutive polls and every one must be silent. **A window is IMMUTABLE** — bounds, cause, counts and ids are snapshotted at detection and never revised, because the queue pauses then and nothing else writes outcome records; `cause` cannot change mid-window either, so a session expiry followed by a key rotation arrives as two windows (both confirmed against the platform's source on 2026-08-23, after this file assumed a growing count). Keeping the count out of the dedupe key stays deliberate anyway: a future platform that revised a figure could then never turn it into a second alarm. Three fields must be read carefully. **`ok !== true` is NO INFORMATION** and takes the quiet fallback — the same trap Students found on `/groups`, where an empty list on a 429 is indistinguishable from health; an *absent* `ok` is tolerated because 0.52.0 shipped the endpoint before the field. **An unrecognized `cause` reads as `unknown`** (the platform says more will be added) and the admin sentence is chosen from a map with a fallback, never switched on — `key-rejected` in particular is not about the phone, so telling that masjid to re-link it would be wrong. **`truncated` is believed when set and INFERRED when `ids.length < count`**, so we can never imply we accounted for every message when we did not. `whatsappSuspect()` polls it hourly plus once ~90s after boot, and **an absent, 404, throttled or malformed answer is `[]` — "nothing to worry about", never "assume a gap"**; the loud fallback would false-alarm every masjid on an older platform for ever. **We deliberately do NOT resend, and that is a domain answer:** every WhatsApp message this app sends is an admin notice about something already durably recorded, no donor is ever messaged, `donation` is the highest-volume event by far (re-sending a day of it to a freshly re-linked number is exactly the ban risk), `paymentFailed` is `burst()`-gated so a still-broken site re-raises it by itself while a fixed one would get a false alarm, and `tuitionFailed`'s real remedy is the three-day record-payment outbox. So we raise **one** notification per window whatever its count (`whatsappGap`, seventh event / eighth alert id), and keep it on the settings screen afterwards because "did I miss a refund last Tuesday?" is asked days later. **What `ids` bought us is detail, not a resend:** `WhatsAppEventOutcome.msgId` keeps the id from the `202` so `eventsForMessageIds` can turn reported ids back into event NAMES — "at least one was the refund notice" is the difference between an admin shrugging and an admin checking. **Matching is by id ONLY, with no timestamp fallback**: an interval-overlap test against `[from,to]` has a false positive the platform's holding behaviour creates — a message queued during an outage and delivered fine after the re-link still falls inside the window. The platform recommended overlap on 2026-08-22 and withdrew it on 2026-08-23 after Display found that. A complete id list is authoritative in both directions (a message not named is a message not lost), and where there are no ids we say only the count. It is **partial by construction** (we keep the newest outcome per recipient+event, a health indicator and not a message log, so a window holding forty donation notices leaves one id to match), so the platform's `count` stays the figure we quote and every sentence says "at least" and "there may have been others". Never present the matched list as the full story. The id is stripped from the settings view — the panel renders a state and a sentence, and an opaque platform id in the DOM is a question somebody has to answer later for no benefit. `store.addWhatsAppGap` is check-and-write in one call, keyed on the window BOUNDS and not the count — the platform re-reports an open window on every poll and its count grows, and re-alarming because 9 became 11 is noise about something already said.
  - **THE PACING IS OURS NOW (platform 0.51.1).** The platform used to refuse to send too much: randomised 6–20s gaps, per-recipient and per-group cooldowns, hourly/daily caps, quiet hours, a warm-up ramp. **All of it is gone** — a message leaves within seconds — and what has not changed is that ban risk attaches to the masjid's NUMBER, that the number is shared with every app on the box, and that a blocked number cannot be recovered. So the bound lives here, and in v0.44.0 it had to change shape. `makeSendBudget` still caps each **destination** at 20/hour, but a masjid may now list several numbers, so "one message per event, never a loop over recipients" is no longer true — a recipient list IS a loop, and the per-destination cap alone would have quietly become 100/hour the moment that shipped. **A second budget therefore caps the gateway as a whole at 40/hour** (`waTotalBudget`, keyed on one constant), checked AFTER the per-destination one so a single destination cannot spend the shared ceiling without also spending its own. Both feed the same held-back figure, which is **counted and shown on the settings screen** — an admin whose phone went quiet has to be able to find out it was deliberate. **`minAmount` was the other half and it is GONE, removed in v0.44.0 on Hasan's explicit instruction with the trade put to him first.** It was a site-wide "only tell me about donations of at least X" floor beside the donation row. What replaces it is per-recipient and explicit: `donation` is one tick in the matrix, a new email address does not start on it, and a new WhatsApp row starts on nothing. What is genuinely lost is "tell me, but only the big ones" — the answer is now all of them or none, on a per-person basis. The risk it used to cover is unchanged and is recorded at `NOTIFY_DEFAULT`: the `donation` alert fires per transaction, and alert mail shares a rate-limit bucket with `payment-failed`, the one that means nobody can give at all. If a masjid reports being buried, re-adding a single site-wide floor is the smaller change than reinstating the old per-event field.
  - **A REFUSAL IS NOT A LOSS, and must never be logged as one.** A 4xx from `/api/fabric/whatsapp` carries a sentence written for an admin — *that group is no longer approved*, *that phone number needs a country code*, *that is the number WhatsApp is linked to* — while a 429/5xx is an outage worth nothing but a log line. Collapsing the two is what made a refused message indistinguishable from a vanished one, and it made the platform's own queue bug look like ours. `sendWhatsApp` returns `refused` separately, and every refusal is stored against its event (`store.setWhatsAppOutcome`) and rendered on that event's block.
  - **Ask what became of it, once (platform 0.51.1+, gated on `status.outcomes`).** `GET /api/fabric/whatsapp/status/<id>` answers `queued|sent|failed|expired`; a 404 means unknown, another app's, an evicted record, or a platform without the endpoint, and **none of those is evidence of failure**. Records are the most recent **500 per app, kept 24 hours** (they were 200 *shared* until platform 0.51.1-dev.8, so a busy app could evict everyone's); status reads have their own 600/min budget, separate from sending. We ask once, ~45s after queueing, never in a loop. Like `media`, an absent `outcomes` field means **no**.
  - **`sent` is a success state, so never key "already handled" on a `queued` row.** Display hit this: its dedupe asked "is there a queued entry?", which meant "handled" only while `queued` was the *only* success — the moment delivery was confirmed the check stopped matching and the next tick sent a duplicate. **Success turning into a duplicate.** This app is structurally clear (it never retries a send and keys nothing on a state), and any future dedupe here must key on the message **id**. `expired` counts as a failure — the recipient still has nothing.
  - **An event that fires per EXTERNAL failure needs its own gate, and the gate must count.** Kiosk found this: its per-PaymentIntent alert had no natural bound, and the platform's old 60-second cooldown had been silently absorbing it. Ours are `paymentFailed` (both sites) and `tuitionFailed` — one Stripe outage or one misconfigured Students fails *every* attempt, so a Friday is one message per person who tried. `burst(key, HOUR)` gates them and reports what it swallowed (*"This has happened 41 more times since the last message."*), because "a payment failed" once cannot tell one unlucky donor from everybody. The lost-donation sweep is different and is **batched, not gated**: it can legitimately find 25 real recoveries in a pass, so it collects them and raises one notification with the count and total — gating there would discard real money news.
  - **Minimum platform versions: sending `0.51.0+`, message status and the durable queue `0.51.1+`, suspect-window reporting `0.52.0+`, and its `cause`/`ids`/`ok` fields plus 7-day retention `0.51.1-dev.13+`.** Keep the docs saying so — each endpoint simply 404s on an older platform, which is why every one of them is capability-gated or fails to the quiet answer rather than being assumed.
  - **One recipient per call, and never a client-chosen target.** A group id comes only from `GET /api/fabric/whatsapp/groups` (the ones the *platform* admin approved — not the gateway's own list, which names every group the masjid's phone is in) and is re-verified against that list before we save it. A phone number is normalized with `toWhatsAppDigits`, which **refuses a number with no country code rather than guessing one** — a guess would send the masjid's donation figures to a stranger who happens to hold that number in the platform's default country.
  - **THE COUNTRY IS CHOSEN, NEVER INFERRED (v0.44.0).** `toWhatsAppDigits`' leading-zero rule catches a British admin typing `07700900123`, because no E.164 country code begins with a zero. **It cannot catch the American equivalent, and for a year it did not.** A bare ten-digit `3135550142` has no leading zero, so it passed — and read as E.164 that is **+31, the Netherlands**, number 35550142. A masjid in Michigan would have been sending its donation figures to a stranger in Amsterdam and nothing on any screen would have looked wrong. The fix is not a cleverer parser: it is that a single text box cannot carry a fact it was never given. The panel now takes the **country from a dropdown** and the national number separately, the wire carries `dialId` + `national`, and `server/src/phone.ts`'s `toE164` composes the two — so the dial code is something an admin picked from a list. `toWhatsAppDigits` stays exactly as it is and stays the **final gate**; `toE164` feeds it and never replaces it. Three things must not be undone: the panel must never offer a single combined number field again (that is the bug); a trunk zero may be stripped only **after** a country is known (which is what makes it a correction rather than a guess); and an already-prefixed paste must be de-duplicated rather than doubled, since `1` + `13135550142` is a plausible twelve-digit number belonging to nobody. `phone.test.ts` pins all of it, including the original bug as a regression witness.
...
- All user-facing strings via i18n; all colors/spacing via tokens.
...
Builds via the documented commands and `docker compose up -d`; `tsc`/lint clean; works in light + dark and matches the dashboard theme when embedded; honors `prefers-reduced-motion`; admin behind auth; **Stripe secret never reaches the client**; one-time donations work **with no public ingress**; manifest + compose pass the APP_MANIFEST_SPEC rules; friendly wording; no raw error reaches the user.
